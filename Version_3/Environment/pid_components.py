"""
Componentes PID reutilizables para control y reinforcement learning - VERSION CORREGIDA
...
class PIDController:
    ...
    def compute(self, setpoint: float = None, process_value: float = None, error: float = None) -> float:
        """
        Calcular salida PID con anti-windup.
        
        Args:
            setpoint: Punto de ajuste (opcional si se proporciona error)
            process_value: Valor del proceso (opcional si se proporciona error)
            error: Error actual (setpoint - pv) - calculado automáticamente si se dan setpoint y process_value
        
        Returns:
            control_output: Señal de control dentro de output_limits
        """
        # Calcular error si se proporcionan setpoint y process_value
        if error is None:
            if setpoint is None or process_value is None:
                raise ValueError("Debe proporcionar 'error' O ambos 'setpoint' y 'process_value'")
...
class DeltaPIDActionSpace:
    ...
    def set_pid(self, kp: float, ki: float, kd: float) -> None:
        """
        Establecer PID directamente (útil para validación).
        ...
        Args:
            ...
            kd: Ganancia derivativa
        """
        self.current_pid = np.array([kp, ki, kd], dtype=np.float32)
        
        # Aplicar límites
        for i in range(3):
            min_val, max_val = self.limits[i]
            self.current_pid[i] = np.clip(self.current_pid[i], min_val, max_val)
