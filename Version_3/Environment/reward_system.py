...
class AdaptiveRewardCalculator:
    ...
    Args:
        ...
        dead_band: Banda muerta aceptable alrededor del setpoint
    """
    ...
    def calculate_global_reward(self, pv_targets, sp_targets, ranges):
        """
        Calcular recompensa global basada en múltiples variables objetivo.
        
        Args:
            pv_targets: Array de valores actuales de las variables objetivo
            sp_targets: Array de setpoints de las variables objetivo
            ranges: Array de rangos aceptables alrededor del SP
        
        Returns:
            Recompensa global normalizada en [0, 1]
        """
        errores_normalizados = []
        
        for i in range(len(sp_targets)):
            rango = ranges[i]
            sp = sp_targets[i]
            pv = pv_targets[i]
            ...
            # Normalizar
            pv_norm = (pv - lower) / rango
            sp_norm = (sp - lower) / rango  # Siempre 0.5
            
            # Error normalizado
            error_norm = abs(pv_norm - sp_norm)
            
            # Penalización cuadrática
            error_penalizado = error_norm * error_norm
            
            errores_normalizados.append(error_penalizado)
        
        # Promedio de errores
        error_promedio = sum(errores_normalizados) / len(errores_normalizados)
        
        # Convertir a recompensa
        recompensa_global = 1.0 - error_promedio
        
        # Clip [0, 1]
        recompensa_global = np.clip(recompensa_global, 0.0, 1.0)
        
        return float(recompensa_global)